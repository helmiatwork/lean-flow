...
| **designer** | `frontend-design:frontend-design` → `superpowers:executing-plans` → `superpowers:test-driven-development` → `superpowers:verification-before-completion`, plus the design-quality cluster: `better-interface` (entry point) → `better-ui` / `better-typography` / `better-colors` / `better-layout` / `better-accessibility` / `better-writing`; and framework/specialist skills as the stack needs: `react-best-practices`, `react-view-transitions`, `composition-patterns`, `web-design-guidelines`, `dataviz` | sonnet | Step has frontend work. Implements UI/components, writes tests, commits to step branch. Invokes `better-interface` first on any non-trivial screen. **Stops before PR** — fixer opens PR and manages review. |
