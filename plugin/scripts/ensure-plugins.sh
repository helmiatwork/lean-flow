...
if [ "$sp_added" = true ] && [ "$cav_added" = true ]; then
  ...
  MSG="Companion plugin enabled: caveman (token-compressed mode). Will be active on next session start. To disable: set LEAN_FLOW_ENABLE_CAVEMAN=false or set enabledPlugins.\"caveman@caveman\" to false in ~/.claude/settings.json."
fi
