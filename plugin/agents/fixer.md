...
11. **Plan checklist write-back (conditional)** — If the dispatch prompt includes `plan_path: <absolute-path>` AND the file at that path contains `- [ ]` markdown checkboxes, after each successful step commit, edit the plan file replacing the `- [ ]` for the just-completed step heading with `- [x]`. Use exact heading text matching from the dispatch prompt's step list. (Note: layer 3 hook provides auto-detection backstop; this step handles explicit plan_path dispatch.)
