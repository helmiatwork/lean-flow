...
| **Hook** | Event-triggered shell command registered in `plugin/hooks/hooks.json`. Lifecycle events: SessionStart, PreToolUse, PostToolUse, Stop, PostCompact, UserPromptSubmit. | `block-protected-push.sh` fires on PreToolUse(Bash) and blocks push to main. |
| **Plan** | Multi-step implementation contract created via `superpowers:writing-plans` skill. Executed via dispatcher → agents per step. Plans live in `~/.claude/plans/`. | A 5-step feature plan with step files + skeleton for user to mark progress. |
