...
SELF_PATH="$HOME/Library/Application Support/SwiftBar/Plugins/claude-usage.5s.sh"
...
if [ -f "$CACHE_FILE" ]; then
  ...
  session_pct="?"; week_all_pct="?"; week_sonnet_pct="?"
  session_reset="?"; week_all_reset="?"; week_sonnet_reset="?"
fi
...
if [ -f "$BLINK_FLAG" ]; then
  flag_age=$(( $(date +%s) - $(stat -f%m "$BLINK_FLAG" 2>/dev/null || echo 0) ))
  if [ "$flag_age" -lt 10 ]; then blink=true; else rm -f "$BLINK_FLAG"; fi
fi
...
if [ -f "$CACHE_FILE" ]; then
  ...
  updated_epoch=$(stat -f%m "$CACHE_FILE" 2>/dev/null || stat -c%Y "$CACHE_FILE" 2>/dev/null || echo 0)
  now_epoch=$(date +%s)
